...
mack_creek_lengths <- mack_creek_vertebrates %>% 
  clean_names() %>% 
  select(year:sampledate) %>% #keep only these columns
  filter(section == "CC") %>% 
...
df <- tribble(
  ~name, ~burritos_consumed, ~siblings,
  "Mia", 2, 3,
  "Paloma", 3, 4.4,
  "Halina", 10, 5.1,
  "Joe", 21, 1
...
ggplot(data = df, aes(x = burritos_consumed, y = siblings))+
  geom_point(color = "purple", shape = 12) 
#color and shape should not be in aes() unless they are related to a variable 
